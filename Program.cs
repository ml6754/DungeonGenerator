...
namespace DungeonGenerator
{
    class Dungeon
    {
        ...
        public static Level pathLevel(Level l)
        {
            ...
            int removerange = l.rooms.Length / 4;
            Random toremove = new Random();
            int remcount = toremove.Next(1, removerange + 1);
            List<int> roomsremoved = new List<int>();
            int r = random_except_list(l.rooms.Length, roomsremoved);
            if (remcount > 0)
            {
                for (int w = 0; w < 4; w++) {
                    for (int k = 0; k < l.paths.Count; k++)
                    {
                        if ((l.paths[k].a == r) || (l.paths[k].b == r))
                        {
                            l.paths.Remove(l.paths[k]);
                            break;
                        }
                    }
                }
                for(int k = 0; k < l.rooms.GetLength(0); k++)
                {
                    for(int j = 0; j < l.rooms.GetLength(1); j++)
                    {
                        if (l.rooms[k, j] == r)
                        {
                            roomsremoved.Add(r);
            ...
            if (remcount > 0)
            {
                r = random_except_list(l.rooms.Length, roomsremoved);
                for (int w = 0; w < 4; w++)
                {
                    for (int k = 0; k < l.paths.Count; k++)
                    {
            ...
            if(roomsremoved.Count == 2){
                for (int k = 1; k < l.rooms.Length + 1; k++)
                {
                    int count = 0;
                    for(int j = 0; j < l.paths.Count; j++)
                    {
                        if ((l.paths[j].a == k) || (l.paths[j].b == k))
                        {
                            count++;
                        }
                    }
                ...
                for (int k = 0; k < l.rooms.GetLength(0); k++)
                {
                    for (int j = 0; j < l.rooms.GetLength(1); j++)
                    {
                        if(roomsremoved.Last() == l.rooms[k, j])
                        {
                            l.rooms[k, j] = 0;
                        }
                    }
            ...
            int randroot = random_except_list(l.rooms.Length, roomsremoved);
            int i = 0;
            int smallest = 100;
